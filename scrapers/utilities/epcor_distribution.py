"""
epcor_distribution.py -- Scraper for EPCOR Distribution rates (Alberta).

EPCOR Distribution Inc. is the regulated electricity distribution utility
serving the City of Edmonton and surrounding area.

Official source:
  https://www.epcor.com/products-services/power/rates-tariffs/Pages/default.aspx
...
class EPCORDistributionScraper(BaseScraper):
    """Scrape EPCOR Distribution electricity distribution rates for Edmonton."""
    ...
    def scrape(self) -> list[TariffRecord]:
        """
        Attempt to scrape live EPCOR Distribution rates.
        ...
        if live_records:
            ...
            self.logger.info(
                "Successfully scraped %d EPCOR Distribution tariffs from live site",
                len(records),
            )
        else:
    ...
    def _try_live_scrape(self) -> Optional[list[TariffRecord]]:
        """Attempt to parse rates from the live EPCOR website."""
        try:
            html = self.fetch_page(SOURCE_URL)
            # TODO: implement HTML parsing once page structure is verified
            return None
        except Exception as e:
            self.logger.warning("Could not fetch EPCOR Distribution page: %s", e)
            return None
